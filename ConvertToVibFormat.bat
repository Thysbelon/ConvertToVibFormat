...
FOR %%i IN (*.wav *.mp3 *.m4a *.ogg *.flac *.aif *.aiff *.aac *.dts *.wma *.wma1 *.wma2 *.alac) DO (
   ...
   "C:\Users\dnfer\Saved Games\mame\chdman" createcd -i "temp.cue" -o "%%~ni.chd"
   echo off
   ...
   echo off
   rem del "%%~nxi"
   echo If you want this script to delete all audio files after processing them, uncomment the line above.
)
@echo off
echo.
echo.
